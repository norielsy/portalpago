...
function mail_error($data){

		//$mail = "cjimenez@mediasoft.cl";
	$mail = "vladimir.pasten@gmail.com";
        $subject = 'Error en lectura de email de transferencia.';
        $from = 'no-reply@portaldepagos.cl';
		...
		$html .= "	</head>";
		$html .= "	<body>";
		$html .= "		<h2>Hubo un error en la lectura de un mail de transferencia</h2>";
		...
		$html .= "</html>";
		  ...
		  $headers .= "BCC: <cjimenez@mediasoft.cl>\n"; //aqui fijo el BCC

		if (mail($mail,$subject,$html, $headers)){
...
//				$sql = "DELETE FROM transferencias_info ";

//				$conexion->query($sql);  //or die('Error on trying to insert data: ' . mysql_error());
...
//$emails = imap_search($gmail_inbox,'ALL');
$new_message = true;

print "Leyendo mensajes, cantidad: " . $n_msgs;


//foreach($n_msgs as $emails) {
...
********************************************/
			...
			if ($host == "santander.cl"){
				...
				//Para obtener los siguientes datos:
					...
					if (!strpos($rut_destinatario,"-"))  {
						$aux = substr($rut_destinatario, 0, strlen($rut_destinatario)-1);
						$dig_ver = substr($rut_destinatario, strlen($rut_destinatario)-1, strlen($rut_destinatario));
						$rut_destinatario = $aux . "-" . $dig_ver;
					}
...
********************************************/
			...
			if ($host == "bci.cl"){
				...
				$nombre_destinatario = str_replace("*", "", $nombre_destinatario);
				$nombre_destinatario = str_replace("=", "", $nombre_destinatario);
				$nombre_destinatario = str_replace("ESTIMADO(A)", "", $nombre_destinatario);
				...
				//Para obtener los siguientes datos:
					...
					//Primera ocurrencia: Banco de destino
					$pos1 = strpos($msg , "Content-Transfer-Encoding: 7bit");
					$txt = substr($msg, $pos1, 2000);;
					$pos1 = strpos($txt , "fondos hacia su cuenta del banco");
					$pos1 += 32;
					$txt = substr($txt, $pos1, 1000);
					//$pos2 = strpos($txt , "Monto transferido");
					$pos2 = strpos($txt , ". El detalle de esta");
					//$pos2 = $pos2 - 12;
...
*******************************************/
			...
			if ($host == "bancochile.cl"){
				...
				$nombre_depositante = obtener_campo($msg, "informamos que", "le ha transferido", 15, 0);
				print ("<BR>(1) " . $nombre_depositante);
				...
				$nombre_banco = obtener_campo($msg, "Banco", "Fecha", 6, 0); 
//print ("<BR>" . $temp);
				//$nombre_banco = obtener_campo($temp."***", " de ", "***", 4, 0);
				print ("<BR>(3) " . $nombre_banco);
					...
					//Segunda ocurrencia: Cuenta de destino
					$cuenta_destino = obtener_campo($msg, "Cuenta", "Banco", 6, 0);
					print ("<BR>(4) " . $cuenta_destino);
					...
					$rut_destinatario = "";
					$rut_destinatario = obtener_campo($msg, "Rut", "Cuenta", 3, 0);
					print ("<BR>(5) ".$rut_destinatario);


					//Cuarta ocurrencia: Monto operación
					$monto_operacion = obtener_campo($msg, "Monto", "ID", 5, 0);
				...
				//Para obtener La glosa:
				$glosa = "";
				$glosa = obtener_campo($msg, " Mensaje ", "Por tu seguridad, este mensaje no tiene enlace ", 9, 0);
				print ("<BR>(7) " . $glosa);
...
//print ("<BR>" . $temp);
				...
				print ("<BR>(3) " . $nombre_banco);
					...
					//Cuarta ocurrencia: Monto operación
					$tmp = limpiar_cadena($msg);
print ("<BR>() $tmp ()<BR>");					
					$monto_operacion = obtener_campo($tmp, "Mon to Transa", "090909", 26, 0);

					if (strlen($monto_operacion)>20){
						//buscamos el monto de otra forma:
						$monto_operacion = obtener_campo($tmp, "Monto Transa", "090909", 25, 0);						
			...
			$glosa = obtener_campo($tmp, "Asu nto", "mero de", 7, 9);
			$glosa = str_replace("=", "", $glosa);
			$glosa = trim($glosa);
...
$sql11 = $sql;
				...
				print "ID transaccion: ". $id_transaccion;
				if($id_transaccion != "codigo no encontrado"){
					$tipo_pago = obtener_tipo_a_pagar($id_transaccion);
print "tipo_pago: -". $tipo_pago."-<br>";					
					if($tipo_pago == "co"){
						//cobros individual
						$sql = "SELECT idCobros 
								FROM cobros P
								INNER JOIN usuarios U ON P.idUsuarios = U.idUsuarios
								WHERE idunico_pago = '".$id_transaccion."' AND monto = '".$monto."' ";

						if ($host != "bci.cl")
							$sql .= " AND U.rut = '".$buscar_rut."'";
						$sql .= "  AND pagado = 0 LIMIT 1";
								
						$rs = $conexion_portal->query($sql);// or die('Error (1) : ' . mysql_error());
print "2.-".$sql."<br>";						
						//if($rs->num_rows > 0){

						if ($row = $rs->fetch_assoc()){//mysql_fetch_array($rs);
							$id = $row['idCobros'];
