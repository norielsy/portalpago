...
//print ("<BR>" . $temp);
			...
			if ($bank!= ""){
				...
				mysql_query($sql,$conexion)  or die('Error on trying to insert data: ' . mysql_error());
				...
				if($id_transaccion != "codigo no encontrado"){
					...
					if($tipo_pago == "co"){
						...
						$rs = mysql_query($sql, $conexion_portal) or die('Error (1) : ' . mysql_error());
						if(mysql_num_rows($rs) > 0){
							...
							$sql = "UPDATE cobros 
									...
									WHERE  idCobros = '$id'";
							mysql_query($sql, $conexion_portal) or die('Error (2): ' . mysql_error());	
							$pagado = true;		
						}else{
